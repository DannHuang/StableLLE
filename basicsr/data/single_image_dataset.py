...
class SingleImageNPDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        latent_np = torch.from_numpy(latent_np).float().unsqueeze(0)
        latent_np = latent_np.to(img_gt.device)
        # normalize
        if self.mean is not None or self.std is not None:
            normalize(img_lq, self.mean, self.std, inplace=True)
            normalize(img_gt, self.mean, self.std, inplace=True)
        return {'lq': img_lq, 'lq_path': lq_path, 'gt': img_gt, 'gt_path': gt_path, 'latent': latent_np[0], 'latent_path': np_path}
