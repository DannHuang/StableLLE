...
class LatentDiffusion(DDPM):
    ...
    def shared_step(self, batch, **kwargs):
        ...
        if self.use_ori_img:
            encoder_posterior = self.encode_first_stage(batch['hint'].permute(0,3,1,2))
            low_light_feat = self.get_first_stage_encoding(encoder_posterior).detach()
        loss = self(x, c,low_light_feat=low_light_feat)
        return loss

    def forward(self, x, c, return_output=False,low_light_feat=None,*args, **kwargs):
        t = torch.randint(0, self.num_timesteps, (x.shape[0],), device=self.device).long()
        ...
        return self.p_losses(x, c, t,return_output=return_output, low_light_feat=low_light_feat,*args, **kwargs)

    def apply_model(self, x_noisy, t, cond, return_ids=False,low_light_feat=None):
        ...
        x_recon = self.model(x_noisy, t, **cond,low_light_feat=low_light_feat)
    ...
    def p_losses(self, x_start, cond, t, noise=None,return_output=False,low_light_feat=None):
        noise = default(noise, lambda: torch.randn_like(x_start))
        x_noisy = self.q_sample(x_start=x_start, t=t, noise=noise)
        # model_output = self.apply_model(x_noisy, t, cond)
        model_output = self.apply_model(noise, t, cond,low_light_feat=low_light_feat)
...
class DiffusionWrapper(pl.LightningModule):
    ...
    def forward(self, x, t, c_concat: list = None, c_crossattn: list = None, c_adm=None,low_light_feat=None):
        if self.conditioning_key is None:
            ...
            if hasattr(self, "scripted_diffusion_model"):
                ...
                out = self.diffusion_model(x, t, context=cc,low_light_feat=low_light_feat)
