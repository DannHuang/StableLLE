...
from ...modules.attention import SpatialTransformer, WaveletTransformer
    ...
    def forward(
        ...
        for layer in self:
            ...
            if isinstance(module, TimestepBlock) and not isinstance(
                ...
                x = layer(x, context)
            elif isinstance(module, WaveletTransformer):
...
class ResBlockDual(TimestepBlockDual):
    ...
    def __init__(
        ...
        assert kernel_size==3   # TODO: for one-time testing
    ...
    def forward(self, x: th.Tensor, emb: th.Tensor, s_cond: th.Tensor) -> th.Tensor:
        """
        Apply the block to a Tensor, conditioned on a timestep embedding.
        :param x: an [N x C x ...] Tensor of features.
        :param emb: an [N x emb_channels] Tensor of timestep embeddings.
        ...
        """
        if self.use_checkpoint:
            return checkpoint(self._forward, x, emb, s_cond)
        else:
...
class DualUNetModel(nn.Module):
    ...
    def __init__(
        ...
        spatial_transformer_attn_type: str = "softmax",
        adm_in_channels: Optional[int] = None,
        hq_code_resolutions=None,           # for CodeDiff [4,2,1]
        semb_channels=None,                  # 256
    ):
        super().__init__()
        ...
        for level, mult in enumerate(channel_mult):
            for nr in range(self.num_res_blocks[level]):
                if ds in hq_code_resolutions:
                    layers = [
                        ResBlockDual(
                            ch,
                            time_embed_dim,
                            dropout,
                            semb_channels=self.semb_channels,
                            out_channels=mult * model_channels,
                            dims=dims,
                            use_checkpoint=use_checkpoint,
                            use_scale_shift_norm=use_scale_shift_norm,
        ...
        **kwargs,
        ...
        for module in self.input_blocks:
            ...
            if last_h.shape[-1] != h.shape[-1]:
                results[str(last_h.shape[-1])] = last_h
                # result_list.append(last_h)
        h = self.middle_block(h, emb, context)
        results[str(h.shape[-1])] = h
