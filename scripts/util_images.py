...
class ImageSpliterTh:
    def __init__(self, im, pch_size, stride, sf=1, gather_method='mean'):
        '''
        Input:
            im: n x c x h x w, torch tensor, float, low-resolution image in SR
            pch_size, stride: patch setting
            sf: scale factor in image super-resolution
            gather_method: mean or gaussian
        '''
        assert stride <= pch_size
        self.stride = stride
        self.pch_size = pch_size
        ...
        if gather_method == 'mean':
            self.weights = torch.ones([1, self.configs.model.params.channels, pch_size, pch_size], dtype=im.dtype, device=im.device)
        elif gather_method == 'gaussian':
            self.weights = self._gaussian_weights(pch_size, pch_size, 1)
        else:
    ...
    def _gaussian_weights(self, tile_width, tile_height, nbatches):
        ...
        var = torch.tensor(0.01, dtype=torch.float32)
        midpoint = torch.tensor((latent_width - 1) / 2, dtype=torch.float32)  # -1 because index goes from 0 to latent_width - 1
        x_probs = [(-(x-midpoint)*(x-midpoint)/(latent_width*latent_width)/(2*var)).exp() / (2*np.pi*var).sqrt() for x in range(latent_width)]
        midpoint = torch.tensor((latent_height - 1) / 2, dtype=torch.float32)
        y_probs = [(-(y-midpoint)*(y-midpoint)/(latent_height*latent_height)/(2*var)).exp() / (2*np.pi*var).sqrt() for y in range(latent_height)]

        weights = torch.einsum("i,j->ij", torch.tensor(y_probs), torch.tensor(x_probs))
        return torch.tile(weights.to(self.im_ori.device), (nbatches, self.im_ori.size(1), 1, 1))
    ...
    def update(self, pch_res, index_infos):
        ...
        self.im_res[:, :, h_start:h_end, w_start:w_end] += pch_res*self.weights
        self.pixel_count[:, :, h_start:h_end, w_start:w_end] += self.weights
